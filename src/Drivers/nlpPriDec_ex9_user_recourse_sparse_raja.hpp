...
public:
                 ...
                 double* cons)
  {
    ...
    const auto *d_xi_ = xi_;
    const auto d_nx_ = nx_;
    const auto d_nS_ = nS_;
      ...
      RAJA_LAMBDA(RAJA::Index_type irow)
    {
      ...
      if(con_idx<m-1) {
        ...
        assert(con_idx==m-1);
        cons[m-1] = (1-x[0]+d_xi_[0])*(1-x[0]+d_xi_[0]);
     //   RAJA::forall<ex9_raja_exec>(RAJA::RangeSegment(1, nS_),
     //     RAJA_LAMBDA(RAJA::Index_type i)
     //	{
        for (int i=1; i< d_nS_; i++) {
          cons[m-1] += (x[i] + d_xi_[i])*(x[i] + d_xi_[i]);
        }
        //});
        //RAJA::forall<ex9_raja_exec>(RAJA::RangeSegment(nS_, nx_),
        //  RAJA_LAMBDA(RAJA::Index_type i)
	//{
        for (int i=d_nS_; i< d_nx_; i++) {
	  cons[m-1] += x[i]*x[i];
        }
        //});
      }
    ...
    if(MJacS!=NULL) {
       ...
       [=] __device__ (RAJA::Index_type itrow)
      {
        ...
        if(con_idx<m-1) {
          ...
          assert(itrow==m-1);
	  MJacS[2*(m-1)] = -2*(1-x[0]+d_xi_[0]);
          //nnzit[0] += 1;
          //cons[m-1] = (1-x[0]+xi_[0])^2;
	  assert(m>=d_nS_);
	  for(int i=1; i<d_nS_; i++)
	  {
            MJacS[2*(m-1)+i] = 2*(x[i]+d_xi_[i]);
            //nnzit[0] += 1;
	  }
            //cons[m-1] += (x[i] + xi_[i])*(x[i] + xi_[i]);
	  for(int i=d_nS_; i<m; i++)
	  {
            MJacS[2*(m-1)+i] = 2*x[i];
            //nnzit[0] += 1;
            //cons[m-1] += x[i]*x[i];
