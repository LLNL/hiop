...
int main(int argc, char **argv)
{
  ...
  if(one_call_cons)
  {
    my_nlp = new EX6OneCallCons(n_sp, n_de, mem_space);
  }
  else
  {
    my_nlp = new EX6(n_sp, n_de, mem_space);
  }
  ...
  if(selfCheck) {
    if(fabs(obj_value-(-4.999509728895e+01))>1e-6) {
      printf("selfcheck: objective mismatch for EX6 MDS problem with 400 sparse variables and 100 "
