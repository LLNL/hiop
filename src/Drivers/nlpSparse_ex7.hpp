 ...
 *
 *  min   -sum 1/4* { (x_{i}-1)^4 : i=1,...,n} + 0.5x^Tx
 *  s.t.
 *            4*x_1 + 2*x_2 == 10
 *        5<= 2*x_1 + x_3
 *        1<= 2*x_1           + 0.5*x_4 <= 2*n
 *            4*x_1                     + sum{x_i : i=5,...,n} <=4*n
 *        x_1 free
 *        0.0 <= x_2
 *        1.5 <= x_3 <= 10
 *        x_i >=0.5, i=4,...,n
 ...
 *
 *  s.t.  [-inf] <= 4*x_1 + x_3 + 0.5*x_4 <= [ 4 ]                  (rnkdef-con1.1)
 *        [ -4 ] <= 6*x_1 + x_3 + sum{x_i : i=5,...,n}  <= [inf]    (rnkdef-con1.2)
...
{
public:
  Ex7(int n, bool rankdefic_Jac_eq, bool rankdefic_Jac_ineq));
  virtual ~Ex7();
