...
/**
 *  Example of the Augmented Lagrangian solver. The Ipopt's interface is assumed,
 *  therefore we need to wrap the hiop examples in the IpoptAdapter class.
 * 
 *  Relevant options in hiop.options:
 *  ---------------------------------
 *  verbosity_level 3
 *  verbosity_level_subproblem 0
 *  subproblem_solver hiop (or ipopt)
 *  max_iter 200
 *  rho0 1e5
 *  lambda0 1.0
 *  warm_start 0            (works only with ipopt, nlp_in->get_starting_point() needs to init also lambda,z_L,z_U)
 *  quasi_newton_switch_iter -1
 *  tolerance 2e-9
 */
...
{
  printf("hiOp AugLagr driver '%s' that solves a synthetic infinite dimensional problem of variable size. A 1D mesh is created by the example, and the size and the distortion of the mesh can be specified as options to this executable. The distortion of the mesh is the ratio of the smallest element and the largest element in the mesh.\n", exeName);
  printf("Usage: \n");
