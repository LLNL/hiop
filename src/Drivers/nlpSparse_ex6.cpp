...
{
  assert(m==n_cons);
  clow[0]= 10.0;    cupp[0]= 10.0;      type[0]=hiopInterfaceBase::hiopLinear;
...
bool Ex6::eval_f(const long long& n, const double* x, bool new_x, double& obj_value)
{
  assert(n==n_vars);
  obj_value=0.;
  for(int i=0;i<n;i++) obj_value += 0.25*pow(x[i]-1., 4);
...
{
  assert(n==n_vars);
  for(int i=0;i<n;i++) gradf[i] = pow(x[i]-1.,3);
  return true;
}
			     ...
			     const int& nnzJacS, int* iJacS, int* jJacS, double* MJacS)
{
    ...
    assert(nnzJacS == 6 + 1 + n-4);
    assert(iJacS!=NULL && jJacS!=NULL);

    int nnzit = 0, nnzit2 = 0;
    ...
    if(MJacS!=NULL) {
        ...
        assert(nnzit2 == nnzJacS);
    }
}


bool Ex6::eval_Hess_Lagreval_Hess_Lagr(const long long& n, const long long& m,
