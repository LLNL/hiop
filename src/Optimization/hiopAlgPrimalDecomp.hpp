...
struct quadratic_hessian{
  
  quadratic_hessian() : quadratic_hessian(-1) {}
  quadratic_hessian(const int& n)
  {
    n_=n;
    xkm1 = new double[n_];
    skm1 = new double[n_];
...
public:
  ...
  virtual hiopSolveStatus run()
  {
    ...
    //if(my_rank_==0){
      quadratic_hessian*  hess_appx_2 = new quadratic_hessian(n_);
    //}
    ...
    for(int it=0; it<max_iter;it++){
	  ...
	  double alp_temp = hess_appx_2->get_alpha();
        ...
        RecourseApproxEvaluator* evaluator = new RecourseApproxEvaluator(n_,S_,rval,grad_r, 
		                                hess_appx, x_);
        for(int i=0;i<n_;i++) printf("%d x0 %18.12e\n",i,x_[i]);
