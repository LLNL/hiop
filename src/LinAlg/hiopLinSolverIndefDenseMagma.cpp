...
namespace hiop
{
  ...
  int hiopLinSolverIndefDenseMagmaBuKa::matrixChanged()
  {
    ...
    //query sizes
    //
    magma_dsytrf(uplo, N, M.local_buffer(), lda, ipiv_, &info );
  ...
  bool hiopLinSolverIndefDenseMagmaBuKa::compute_inertia(double **MM, 
							 int N, 
							 int *ipiv,
							 int& posEigVal, 
							 int& negEigVal, 
  ...
  bool hiopLinSolverIndefDenseMagmaBuKa::solve(hiopVector& x_)
  {
    ...
    int info;
    DSYTRS(&uplo, &N, &NRHS, M.local_buffer(), &LDA, ipiv_, x->local_data(), &LDB, &info);
    if(info<0) {
