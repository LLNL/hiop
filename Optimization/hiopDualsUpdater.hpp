...
public:
   ...
   * multiplier updating strategy are provided by child classes *
   * - linear (Newton) update in hiopDualsNewtonLinearUpdate
   * - lsq in hiopDualsLsqUpdate
   * The parameters are:
   * - iter_plus: [in/out] on return the duals should be update; primals are already updated, but 
   ...
   * - f,c,d: fcn evals at the updated (primal) iteration
   * - grad_f, jac_c, jac_d: derivatives at the  updated (primal) iteration
   * - search_dir: search direction (already used to update primals, potentially to be used to 
   * update duals)
   * - alpha_primal: step taken for primals (also taken for eq. duals for the linear duals update)
...
public:
  ...
  /* simple linear update of step length alpha_primal in eq. duals yc and yd and step length
